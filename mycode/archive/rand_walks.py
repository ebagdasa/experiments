...
from multiprocessing import Manager, Process, Queue, JoinableQueue, Value, Lock, Pool
...
def generate_random_walks(iterator):
...
if __name__ == '__main__':
    ...
    proc = 16
    pool = mp.Pool(proc)
    callback = pool.map(generate_random_walks, range(16))
    ran = list()
    for call in callback:
        ran.extend(call)
    save_obj(ran, 'ranwalks')

    # for entry in range(CPU_COUNT):
    #     new_proc = Process(target=generate_random_walks, args=(entry, None, sp_m, 20))
